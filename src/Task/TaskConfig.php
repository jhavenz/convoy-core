...
final class TaskConfig
{
    ...
    public function with(
        ...
        if ($priority !== null) $clone->priority = $priority;
        if ($pool !== null) $clone->pool = $pool;
        if ($retry !== null) $clone->retry = $retry;
        if ($timeout !== null) $clone->timeout = $timeout;
        if ($concurrencyLimit !== null) $clone->concurrencyLimit = $concurrencyLimit;
        if ($trace !== null) $clone->trace = $trace;
        if ($tags !== null) $clone->tags = $tags;
        return $clone;
    }
}
